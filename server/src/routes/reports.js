...
router.get('/sales', async (req, res) => {
  try {
    ...
    const summary = {
      ...
      totalProfit: sales.reduce((s, r) => {
        return s + parseFloat(r.totalPrice) - (parseFloat(r.costPrice) * r.qty) -
          parseFloat(r.shippingCost) + parseFloat(r.shippingCharge) - parseFloat(r.discount);
      }, 0)
    };
...
router.get('/products', async (req, res) => {
  try {
    ...
    sales.forEach(s => {
      ...
      productMap[pid].revenue += parseFloat(s.totalPrice);
      productMap[pid].qtySold += s.qty;
      productMap[pid].profit += parseFloat(s.totalPrice) - (parseFloat(s.costPrice) * s.qty);
...
router.get('/customers', async (req, res) => {
  try {
    ...
    const sales = await prisma.sale.findMany({
      where: { status: { not: 'Cancelled' }, customerId: { not: null }, ...dateFilter },
    ...
    sales.forEach(s => {
      ...
      if (!customerMap[cid]) {
        customerMap[cid] = {
          ...
          phone: s.customer?.phone || s.customerPhone,
          city: s.customer?.city || s.customerCity,
          totalSpent: 0,
          orderCount: 0
        };
      }
      customerMap[cid].totalSpent += parseFloat(s.totalPrice);
  ...
  try {
    ...
    if (type === 'sales') {
      ...
      data = sales.map(s => ({
        ...
        'Qty': s.qty,
        'Unit Price': parseFloat(s.unitPrice),
        'Total': parseFloat(s.totalPrice),
        'Cost': parseFloat(s.costPrice) * s.qty,
      ...
      const pnlData = [
        ...
        { Item: 'Shipping Costs', Amount: -shippingCost },
        { Item: 'Gross Profit', Amount: grossProfit },
        { Item: '', Amount: '' },
        { Item: 'EXPENSES', Amount: '' },
        ...Object.entries(expensesByCategory).map(([cat, amt]) => ({ Item: cat, Amount: -amt })),
        { Item: 'Total Expenses', Amount: -totalExpenses },
        { Item: '', Amount: '' },
        { Item: 'Net Profit', Amount: grossProfit - totalExpenses }
      ];
