...
export default function Store() {
  ...
  // Category sections and the best-sellers strip, both built server-side so the ordering
  // rules live in one place. Empty while searching — search results stay a flat grid.
  ...
  useEffect(() => {
    ...
    Promise.all([getStoreInfo(slug), getStoreProducts(slug)])
      .then(async ([infoRes, prodRes]) => {
        ...
        setSections(prodRes.data.sections || []);
        setPopularIds(prodRes.data.popularIds || []);
        if (orderId) {
          try {
  ...
  const loadProducts = (cat, q) => {
    getStoreProducts(slug, { category: cat || undefined, search: q || undefined })
      .then(res => {
        ...
        setPopularIds(res.data.popularIds || []);
      });
  };

  const handleSearch = (q) => { setSearch(q); loadProducts('', q); };

  // The whole catalogue is on the page as sections now, so a category pill scrolls to its
  // section instead of round-tripping to filter. Searching still refetches.
  const sectionAnchor = (title) => 'cat-' + title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
  const handleCategory = (cat) => {
    setCategory(cat);
    if (!cat) { window.scrollTo({ top: 0, behavior: 'smooth' }); return; }
    document.getElementById(sectionAnchor(cat))?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  };
  ...
  // Sections and the popular strip reference products by id, so the tile objects are looked up
  // from the one `products` array rather than being sent twice over the wire.
  const productById = new Map(products.map(p => [p.id, p]));
  const tilesFor = (ids) => ids.map(id => productById.get(id)).filter(Boolean);
  const popularTiles = tilesFor(popularIds);
  const showSections = !search && sections.length > 0;

  // One product tile. Extracted so the flat search grid, the popular strip and every category
  // section render exactly the same card.
  const renderTile = (p) => {
        ...
        {products.length > 0 ? (
          showSections ? (
            <div className="space-y-12">
              {popularTiles.length >= 3 && (
                <section>
                  <h2 className="text-lg font-bold text-slate-800">Popular right now</h2>
                  <p className="text-sm text-slate-500 mb-4">What customers have been buying most</p>
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-5">
                    {popularTiles.map(renderTile)}
                  </div>
                </section>
              )}
              {sections.map(s => (
                <section key={s.title} id={sectionAnchor(s.title)} className="scroll-mt-28">
                  <div className="flex items-baseline justify-between mb-4 pb-2 border-b border-slate-200">
                    <h2 className="text-lg font-bold text-slate-800">{s.title}</h2>
                    <span className="text-xs text-slate-400">{s.productIds.length} item{s.productIds.length === 1 ? '' : 's'}</span>
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-5">
                    {tilesFor(s.productIds).map(renderTile)}
                  </div>
                </section>
              ))}
            </div>
